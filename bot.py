...
# Tu ID de Telegram (para seguridad - solo tú puedes usar el bot)
# Déjalo vacío "" para que cualquiera pueda usarlo
YOUR_TELEGRAM_ID = os.environ.get("YOUR_TELEGRAM_ID", "")

# ─── Sistema de Memoria ───────────────────────────────────────────────────────
def load_memory() -> dict:
    """Carga la memoria persistente del bot"""
...
def save_message(memory: dict, user_id: str, role: str, content: str):
    """Guarda un mensaje en el historial (máximo 20 mensajes)"""
...
- Recordatorios: cuando el usuario mencione fechas u horas, recuérdalo

🔧 COMANDOS QUE PUEDES EJECUTAR (cuando sea apropiado, incluye en tu respuesta):
...
Responde de forma concisa y útil. Máximo 3-4 párrafos salvo que te pidan más detalle."""
    ...
    # Preparar historial (sin timestamps para la API)
    messages = []
    for msg in conversation_history[-10:]:  # Últimos 10
        messages.append({
            "role": msg["role"],
            "content": msg["content"]
        })
    ...
    response = client.messages.create(
        model="claude-haiku-4-5",  # Modelo más económico, sigue siendo potente
        max_tokens=1024,
...
def process_ai_commands(response_text: str, memory: dict) -> str:
    """Procesa los comandos especiales en la respuesta de la IA"""
    import re
...
async def ver_tareas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if completadas:
        msg += f"\n*Completadas hoy:* {len(completadas)} ✅"
...
async def ver_notas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    memory = load_memory()
    notes = memory["notes"][-10:]  # Últimas 10
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        error_msg = f"❌ Error al procesar tu mensaje: {str(e)}"
        if "api_key" in str(e).lower():
            error_msg = "❌ Error de API. Verifica tu ANTHROPIC_API_KEY."
...
def main():
    ...
    if not ANTHROPIC_API_KEY:
        print("❌ ERROR: Falta ANTHROPIC_API_KEY en las variables de entorno")
        return
